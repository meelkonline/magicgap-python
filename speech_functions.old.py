...
def apply_emotions(segments, raw_emotions, multiplier):
    ...
    for segment in segments:
        ...
        if segment['type'] == 'speech':
            ...
            if segment_characters > segment_max_length:
                ...
                split_time_duration = segment_duration / 2
            ...
            for sub_segment in sub_segments:
                ...
                closest_emotion = min(raw_emotions, key=lambda e: abs(
                    e['start_percentage'] / 100 - sub_segment['start_percentage'] * multiplier))

                text = closest_emotion['text']
                # print('-------------------------------')
                # print(sub_segment['start_time'])
                # print(sub_segment['start_percentage'])
                print(closest_emotion)
    ...
    # which emotion is missing, and therefore pieces of texts
        ...
        if raw_emotion['id'] not in used_ids:
            for i in range(len(used_emotions) - 1, -1, -1):
                ...
                if used_emotions[i]['id'] < raw_emotion['id']:
                    # Add each missing emotion text to its  previous saved emotion
...
def phonemize_audio(audio_path, text):
    ...
    multiplier = total_chars / phonemes[1]
    emotions = apply_emotions(phonemes[0], raw_emotions, multiplier)
    ...
    #
    # print('-------------------------------')
    # emotions = extract_texts(emotions)
    # for i, emotion in enumerate(emotions):
    #     print(emotion)
